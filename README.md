...
把“新关注我的”、粉丝列表或评论管理长截图自动切成单条卡片，并合成到猫咪怀里的 Windows 本地工具。
...
- 自动区分关注截图和评论管理截图；评论支持独立“回复”以及“时间地点＋回复”合并锚点。
...
1. 在手机上手动截取“新关注我的”“粉丝指数 > 关注我的人”或“评论管理”完整列表；也支持只截一条横向粉丝卡片。
2. 关注截图尽量保留每行右侧的“…”；评论截图保留顶部筛选栏，并确保每条评论底部的“回复”完整显示。
3. 把 PNG/JPG 截图复制到 `inbox`。
4. 双击 `01_处理一次.bat`。
5. 打开 `output/batches` 中最新的时间文件夹。

同一个 `inbox` 可以同时放多张关注截图和评论截图，程序会逐张自动识别类型。关注与评论使用不同的去重范围；同一位用户的不同评论按评论内容分别生成，相同评论截图再次处理会被跳过。
...
评论功能已经完全接入主流程，关注和评论截图统一使用：

1. 截图时尽量保留每条评论左侧的圆形头像，最后一条最好保留完整的“回复”二字。截图不必包含顶部“评论管理”和筛选栏。
2. 把评论截图放入普通的 `inbox`。
3. 只生成本地成品时双击 `01_处理一次.bat`。
4. 需要生成后上传飞书时双击 `08_批量处理并上传飞书.bat`，检查预览后输入 `UPLOAD`。
5. 打开 `output/batches` 中最新的时间文件夹查看结果。

程序优先使用左侧圆形头像定位每条评论的起点，因此可包含截图最上方和最下方的评论，也能适应文字、图片、表情包导致的可变高度。“回复”及“2分钟前·云南 回复”仍用于判断截图类型和完整性；如果头像检测失败，程序会自动回退到原有的“回复”锚点切割。截图底部的最后一条即使缺少“回复”也会生成，但会进入 `needs_review` 而不自动上传。

```text
output/batches/2026-08-22_20-50-49/
├─ final/          昵称识别可靠的评论成品
...
需要一次放入多张关注或评论截图时，可以把它们全部复制到 `inbox`，再双击 `08_批量处理并上传飞书.bat`。程序会自动识别每张截图的类型，把它们放进同一个新批次，先显示上传预览；只有输入大写 `UPLOAD` 后，才会上传本次新批次。历史批次、重复评论、待复核结果和纯符号占位昵称不会被上传。
...
| `08_批量处理并上传飞书.bat` | 处理 inbox 全部关注和评论截图，确认后只上传本次新批次 |
...
- 评论处理只读取用户主动放入统一 `inbox` 的截图；不使用逆向接口、模拟点击或自动发布。
