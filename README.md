...
`09_评论区截图测试.bat` 仍作为独立测试入口保留：它读取 `comment_inbox`、输出到 `output/comment_batches`，不写入主去重数据库，也不会被飞书同步。日常使用请优先选择统一的 `inbox` 和 `01`/`08`。
...
| `09_评论区截图测试.bat` | 独立测试 comment_inbox，不进入主去重或飞书流程 |
...
- 评论处理只读取用户主动放入 `inbox` 或测试用 `comment_inbox` 的截图；不使用逆向接口、模拟点击或自动发布。
